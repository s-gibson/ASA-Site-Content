...
load("clean_data_2016_2017.RData")
require(rsconnect)
require(shiny)
require(ggplot2)
require(corrplot)
...
# Team.Fantasy.totals.2016_2017 <- unique(Fantasy.2016_2017[c("Week", "Team")])
# Team.Fantasy.totals.2016_2017$Spread <- NA
# Team.Fantasy.totals.2016_2017$Offensive.Fantasy.Points <- NA
# Team.Fantasy.totals.2016_2017$H.A <- NA
# 
# for (i in c(1:nrow(Team.Fantasy.totals.2016_2017))) {
#   Team.Fantasy.totals.2016_2017$Spread[i] <- Fantasy.2016_2017$Actual.Spread[which(
#     Fantasy.2016_2017$Team == Team.Fantasy.totals.2016_2017$Team[i] &
#       Fantasy.2016_2017$Week == Team.Fantasy.totals.2016_2017$Week[i])][1]
#   
#   Team.Fantasy.totals.2016_2017$Offensive.Fantasy.Points[i] <- sum(Fantasy.2016_2017$DK.points[which(
#     Fantasy.2016_2017$Team == Team.Fantasy.totals.2016_2017$Team[i] &
#       Fantasy.2016_2017$Week == Team.Fantasy.totals.2016_2017$Week[i])], na.rm = T)
#   
#   Team.Fantasy.totals.2016_2017$H.A[i] <- as.character(Fantasy.2016_2017$h.a[which(
#     Fantasy.2016_2017$Team == Team.Fantasy.totals.2016_2017$Team[i] &
#       Fantasy.2016_2017$Week == Team.Fantasy.totals.2016_2017$Week[i])])[1]
# }
# Team.Fantasy.totals.2016_2017$H.A[which(Team.Fantasy.totals.2016_2017$H.A == 'h')] <- "Home"
# Team.Fantasy.totals.2016_2017$H.A[which(Team.Fantasy.totals.2016_2017$H.A == 'a')] <- "Away"
...
server <- function(input, output) {
  output$ui <- renderUI({
    switch(input$PlayerOrTeam,
           "Players" =   list(selectInput(inputId = "players", label = "Select players to compare:",
                                          choices = sort(unique(Fantasy.2016_2017$First.Last)), multiple = T),
           ...
           "Teams" = list(selectInput(inputId = "teams", label = "Select team:",
                                      choices = toupper(sort(unique(Fantasy.2016_2017$Team)))),
                          selectInput(inputId = "chart.type", label = "Select chart type:",
  ...
  output$chart <- renderPlot({
    ...
    if (input$chart.type == "DKP Distributions") {
      ggplot(data = Fantasy.2016_2017[which(Fantasy.2016_2017$First.Last %in% input$players),],
      ...
      ggplot(data = Fantasy.2016_2017[which(
        Fantasy.2016_2017$First.Last %in% input$players),]) +
        geom_boxplot(aes(x = factor(Initial.Last, 
                                    levels=levels(factor(Fantasy.2016_2017$Initial.Last[which(
                                      Fantasy.2016_2017$First.Last %in% input$players)]))[
                                        order(tapply(Fantasy.2016_2017$DK.points[which(
                                          Fantasy.2016_2017$First.Last %in% input$players)]/
                                            Fantasy.2016_2017$Total.DKP[which(
                                              Fantasy.2016_2017$First.Last %in% input$players)], 
                                          factor(Fantasy.2016_2017$Initial.Last[which(
                                            Fantasy.2016_2017$First.Last %in% input$players)]), 
                                          median), 
      ...
      ggplot(data = Fantasy.2016_2017[which(Fantasy.2016_2017$Pos %in%
                                            c("QB","RB","WR","TE") &
                                            Fantasy.2016_2017$Team == tolower(input$teams)),]) +
        ...
        ggtitle("Team Position DKP vs. Opponent Average") +
        theme(plot.title = element_text(hjust = 0.5)) +
      ...
      ggplot(data = Fantasy.2016_2017[which(
        Fantasy.2016_2017$Team == tolower(input$teams)),]) +
        geom_point(aes(x = Actual.Points, y = QB.DKP, color = "QB"), size = 0.5) + 
        geom_smooth(aes(x = Actual.Points, y = QB.DKP, color = "QB"), method = 'lm', formula = y ~ x) +
        geom_point(aes(x = Actual.Points, y = RB.DKP, color = "RB"), size = 0.5) + 
        geom_smooth(aes(x = Actual.Points, y = RB.DKP, color = "RB"), method = 'lm', formula = y ~ x) +
        geom_point(aes(x = Actual.Points, y = WR_TE.DKP, color = "WR/TE"), size = 0.5) + 
        geom_smooth(aes(x = Actual.Points, y = WR_TE.DKP, color = "WR/TE"), method = 'lm', formula = y ~ x) +
      ...
      ggplot(data = Fantasy.2016_2017[which(
        Fantasy.2016_2017$Team == tolower(input$teams)),],
        aes(x=Total.DKP, group=H.A, color=H.A, fill = H.A)) +
